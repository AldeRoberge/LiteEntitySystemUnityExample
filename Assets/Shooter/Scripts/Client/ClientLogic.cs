...
namespace Shooter.Scripts.Client
{
    public class ClientLogic : MonoBehaviour, INetEventListener
    {
        ...
        [SerializeField]                                      private SniperTrailEffect _sniperTrailEffectPrefab;
        [SerializeField]                                      private Transform         _entitiesParent;
        [FormerlySerializedAs("_hitEffect")] [SerializeField] private ExplosionEffect   _explosionEffect;

        private Action<DisconnectInfo> _onDisconnected;

        private GamePool<SniperTrailEffect> _sniperTrailsPool;
        private GamePool<ExplosionEffect>         _hitsPool;
        ...
        private int _bytesInPerSecond;
        private int _packetsOutPerSecond;
        private int _bytesOutPerSecond;
        ...
        private SniperTrailEffect ShootEffectConstructor()
        {
            var eff = Instantiate(_sniperTrailEffectPrefab, _entitiesParent);
            eff.Init(e =>
            {
                if (_sniperTrailsPool.Put(e) == false)
                    Destroy(e.gameObject);
            });
            return eff;
        }

        private ExplosionEffect HitEffectConstructor()
        {
            var eff = Instantiate(_explosionEffect, _entitiesParent);
            eff.Init(e =>
            {
        ...
        private void Awake()
        {
            ...
            _sniperTrailsPool = new GamePool<SniperTrailEffect>(ShootEffectConstructor, 200);
            _hitsPool = new GamePool<ExplosionEffect>(HitEffectConstructor, 200);
        ...
        private void Update()
        {
            ...
            if (_secondTimer >= 1f)
            {
                ...
                _bytesInPerSecond = (int)stats.BytesReceived;
                _packetsInPerSecond = (int)stats.PacketsReceived;
                _bytesOutPerSecond = (int)stats.BytesSent;
                _packetsOutPerSecond = (int)stats.PacketsSent;
                stats.Reset();
            }
...
Ping: {_ping}
IN: {_bytesInPerSecond / 1000f} KB/s({_packetsInPerSecond})
OUT: {_bytesOutPerSecond / 1000f} KB/s({_packetsOutPerSecond})
PendingRemove: {_entityManager.PendingToRemoveEntites}";
        ...
        {
            _sniperTrailsPool.Get().Spawn(from, to);
        }
        ...
        void INetEventListener.OnPeerConnected(NetPeer peer)
        {
            ...
            var typesMap = new EntityTypesMap<GameEntities>()
                ...
                .Register(GameEntities.Projectile, e => new PistolBulletProjectile(e));
